...
if (isset($_POST['adds'])){
  if(!empty($_POST['rnames']) && !empty($_POST['raddress']) && !empty($_POST['rcontact']) && !empty($_POST['rtransdesc']) && !empty($_POST['rprice'])){
    ...
    $queryadd=mysqli_query($con,"INSERT INTO regis3 (Transaction_no,name,address,contact_no,Transac_desc,price,admin) VALUES ('".$trans."','".$_POST['rnames']."','".$_POST['raddress']."','".$_POST['rcontact']."','".$_POST['rtransdesc']."','".$_POST['rprice']."','".$_SESSION['anti']."')");
...
if (isset($_POST['viewrec'])){
        $query2 = mysqli_query($con,"SELECT * from regis3");
...
while ($QRb = mysqli_fetch_array($query2)){
    ...
    echo "<td>"."<a href='landing3W.php?id={$QRb['id']}'><input type='button' value='View'></a>"."</td>";
    echo "</tr>";
$counterb++;
}
 echo "</div>";
...
<div class='okies'>
        <h2><?php echo "Registrar System<br>windows 3"?></h2>
        </div>
</div>
